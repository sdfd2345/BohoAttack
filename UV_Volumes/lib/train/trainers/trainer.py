...
class Trainer(object):
    ...
    def save_tps_dataset(self, batch, output):
        datapath = "./data/tps_dataset"
        pose = batch["poses"]
        filename = batch["img_name"][0].split("/")[-1].split(".")[0]
        # Save the tensor
        torch.save(pose, os.path.join(datapath, "poses", filename+".pth" ))
        ...
        with torch.no_grad():
            ...
            Texture_pose = rgb_pred.view(24,tex_size,tex_size,3).detach().cpu().mul(255).byte().numpy()
        ...
        for i in range(0,24):
            ...
            file_path = os.path.join(datapath, "images", filename+"_"+ f"{i:02d}" +".png" )
            # Save the PIL image
    ...
    def val(self, epoch, data_loader, evaluator=None, recorder=None):
        ...
        for batch in tqdm.tqdm(data_loader):
            ...
            with torch.no_grad():
                ...
                # exchange the H and W, other with grid sampling from the Texture_pose will have false
                Texture_pose = rgb_pred.view(24,tex_size,tex_size,3).permute(0,2,1,3).cpu().numpy() * 255
