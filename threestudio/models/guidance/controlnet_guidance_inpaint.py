...
from diffusers import ControlNetModel, DDIMScheduler, StableDiffusionControlNetPipeline, StableDiffusionControlNetInpaintPipeline
...
@threestudio.register("stable-diffusion-controlnet-guidance-inpainting")
class ControlNetGuidanceInpainting(BaseObject):
    @dataclass
    ...
    def prepare_image_cond(self, cond_rgb: Float[Tensor, "B H W C"]):
        if self.cfg.control_type == "normal":
            ...
            #im = Image.fromarray(np.array(detected_map))
            #im.save("images/normal.jpeg")
            ...
            # the joint maps
            detected_map = self.preprocessor(
                cond_rgb,  detect_resolution=512, image_resolution=512,
            )
            control = (
                torch.from_numpy(np.array(detected_map)).float().to(self.device) / 255.0
            )
            im = Image.fromarray(np.array(detected_map))
            im.save("images/joint.jpeg")
            control = control.unsqueeze(0)
            control = control.permute(0, 3, 1, 2)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        mask = mask.permute(0,3,1,2)[:,0,:,:].unsqueeze(0)
        ...
        '''
        mask_image = torch.zeros(size = (1, 1, RH, RW)).float()
        mask_image[0,0, rgb_BCHW_HW8[0,0,:,:]>0] = 1
        mask_image[0,0,0:150,:] = 0 # remove the head
        mask_image[0,0,-150:-1,:] = 0  # remove the shose
        im = Image.fromarray(np.array(mask_image[0][0].numpy(), dtype = np.uint8)*255)
        im.save("images/mask.png")
        '''
        ...
        if self.cfg.use_sds:
            ...
            edit_images = self.pipe(
                    prompt=prompt,
                    generator=generator,
                    image=rgb_BCHW_HW8,
                    control_image=image_cond,
                    mask_image = mask,
                    height = 512,
                    width = 512,
            ...
            # edit_images.save("images/edit_freeview_0.jpg")
            edit_images_tensor = transform(edit_images).unsqueeze(0)
            return {"PIL":edit_images, "edit_images": edit_images_tensor}
...
if __name__ == "__main__":
    ...
    cfg = load_config("configs/debugging/controlnet-normal-inpainting.yaml")
    ...
    rgb_image = cv2.imread("/home/yjli/AIGC/threestudio/dataroot/freeview_0/images/000005.png")
    rgb_image = cv2.resize(rgb_image, (512, 512))
    rgb_image = rgb_image[:, :, ::-1].copy() / 255
    
    
    mask = cv2.imread("/home/yjli/AIGC/threestudio/dataroot/freeview_0/masks/000005.png")
    mask = cv2.resize(mask, (512, 512))
    mask = mask.copy() / 255
    
    cv2.imwrite("images/freeview0_masked.jpg", rgb_image*mask*255)
    
    mask = torch.FloatTensor(mask[:,:,0]).unsqueeze(0).to(guidance.device)
    rgb_image = torch.FloatTensor(rgb_image).unsqueeze(0).to(guidance.device)
    
    prompt_utils = prompt_processor()
    guidance_out = guidance(rgb_image, rgb_image, mask, prompt_utils, prompt = cfg.system.prompt_processor.prompt,
                            negative_prompt =  cfg.system.prompt_processor.negative_prompt )
    edit_image = guidance_out["PIL"]
    edit_image.save("images/edit_freeview_0.jpg")
