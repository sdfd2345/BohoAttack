...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        dst_poses[:3] = cv2.Rodrigues(root_rmtx)[0][:, 0]
        ...
        E_saved = np.linalg.inv(E)
        E_convert_pose = convert_pose(E_saved)
        # E_saved = make_camera_face_opposite_direction(E_saved)
        results.update({
                'K': K,
                'E': E_convert_pose})
