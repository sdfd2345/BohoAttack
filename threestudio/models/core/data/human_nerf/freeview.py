...
def make_camera_face_opposite_direction(extrinsic_matrix):
    ...
    new_rotation_matrix = np.column_stack((-right_vector, -up_vector, forward_vector))
    extrinsic_matrix[:3, :3] = new_rotation_matrix
    return extrinsic_matrix


def move_camera_to_origin(extrinsic_matrix):
    translation_vector = extrinsic_matrix[:3, 3]
    extrinsic_matrix[:3, 3] = np.zeros(3)
    extrinsic_matrix[:3, 3] = -np.dot(extrinsic_matrix[:3, :3], translation_vector)
    return extrinsic_matrix

def rotate_extrinsic_matrix_to_y(extrinsic_matrix):
    rotation_matrix = extrinsic_matrix[:3, :3]
    current_y_axis = rotation_matrix[:, 1]

    desired_y_axis = np.array([0, 1, 0])
    rotation_axis = np.cross(current_y_axis, desired_y_axis)
    rotation_axis /= np.linalg.norm(rotation_axis)

    dot_product = np.dot(current_y_axis, desired_y_axis)
    rotation_angle = np.arccos(dot_product)

    rotation_matrix_new, _ = cv2.Rodrigues(rotation_axis * rotation_angle)
    rotation_matrix[:, 1] = rotation_matrix_new[:, 1]
...
class Dataset(torch.utils.data.Dataset):
    ROT_CAM_PARAMS = {
        ...
        # 'zju_mocap': {'rotate_axis': 'y', 'inv_angle': False},
    ...
    def get_freeview_camera(self, frame_idx, total_frames, trans=None):
        ...
        if self.src_type == "zju_mocap":
            E_before_aligned = self.train_camera['extrinsics']
            E_after_aligned = np.eye(4, dtype = float)
            print("E before aligned:", E_before_aligned)
            #rotation_matrix = np.array([[0,0,1],[1,0,0],[0, -1, 0]])
            rotation_matrix = cv2.Rodrigues(np.array([-np.pi/2.0, -np.pi/2.0,0]))[0].astype('float32')
            E_after_aligned[:3,:] = np.matmul(rotation_matrix, E_before_aligned[:3,:])
            #new_trans = np.array([trans[2],trans[0], -trans[1]])
            #print("E after aligned:", E_after_aligned)
            E = rotate_camera_by_frame_idx(
                    extrinsics=E_after_aligned, 
                    frame_idx=frame_idx,
                    period=total_frames,
                    trans=trans,
    ...
    def load_image(self, frame_name, bg_color):
        ...
        if cfg.resize_img_scale != 1.:
            img = cv2.resize(img, None, 
                             ...
                             interpolation=cv2.INTER_LANCZOS4) # 1024*1024 -> 256*256
    ...
    def __getitem__(self, idx):
        ...
        # estimate the camera transforms through the SMPL model
        ...
        E = apply_global_tfm_to_camera( E = E, Rh=dst_Rh, Th=dst_Th)
        
        c2w = np.linalg.inv(E)
        c2w_xzy = convert_pose(c2w)
        results.update({
                'K': K,
                'c2w_xzy': c2w_xzy})
