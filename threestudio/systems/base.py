...
class BaseSystem(pl.LightningModule, Updateable, SaverMixin):
    ...
    def on_before_optimizer_step(self, optimizer):
        ...
        '''
        total_norm = 0
        parameters = [p for p in self.geometry.density_network.parameters() if p.grad is not None and p.requires_grad]
        for p in parameters:
            param_norm = p.grad.data.norm(2)
            total_norm += param_norm.item() ** 2
        total_norm = total_norm ** (1. / 2)
        print("density grad", total_norm)
        total_norm = 0
        parameters = [p for p in self.geometry.feature_network.parameters() if p.grad is not None and p.requires_grad]
        for p in parameters:
            param_norm = p.grad.data.norm(2)
            total_norm += param_norm.item() ** 2
        total_norm = total_norm ** (1. / 2)
        print("color grad", total_norm)
        '''
