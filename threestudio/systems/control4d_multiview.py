...
class Control4D(BaseLift3DSystem):
    ...
    def training_step(self, batch, batch_idx):
        ...
        out = self(batch) # rendered image (the output of GAN)
        if self.per_editing_step > 0 and self.global_step > self.start_editing_step:
            ...
            if (
                ...
                if self.cfg.guidance_type == "stable-diffusion-controlnet-guidance":
                    input_rgb = out["comp_gan_rgb"] * mask +  origin_gt_rgb * (1-mask)
                    result = self.guidance(input_rgb, origin_gt_rgb, prompt_utils)
                    # output_rgb =  result["edit_images"].detach().cpu() * mask +  origin_gt_rgb * (1-mask)
                    self.edit_frames[batch_index] = result["edit_images"].detach().cpu() * mask 
                elif self.cfg.guidance_type == "stable-diffusion-controlnet-guidance-inpainting":
                    input_rgb = out["comp_gan_rgb"] * mask +  origin_gt_rgb * (1-mask)
                    result = self.guidance(input_rgb, origin_gt_rgb, mask, prompt_utils,
                                           prompt = self.cfg.prompt_processor.prompt,
                                           negative_prompt =  self.cfg.prompt_processor.negative_prompt)
                    self.edit_frames[batch_index] = result["edit_images"].permute(0,2,3,1).detach().cpu()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.save_image_grid(
            f"val_it{self.true_global_step}-{batch['index'][0]}.jpg",
            [
                {
                    "type": "rgb",
                    "img": out["comp_rgb"][0],
