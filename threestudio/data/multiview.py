...
def BLENDER2OPENCV(C2W):
    ...
    mat[1, 1] = -1
    mat[2, 2] = -1
    C2W = torch.matmul(C2W, mat)
    return C2W

# 正交投影 缩放物体到[-1,1] 同时远平面缩放到和近平面相同的大小，将一个台体缩放到长方体
    ...
    def __init__(self, cfg: Any) -> None:
        ...
        for frame in tqdm(frames): 
            ...
            c2w: Float[Tensor, "4 4"] = torch.as_tensor(
                transform_matrix, dtype=torch.float32
            )
            c2w_list.append(c2w)
        ...
        for idx, frame in tqdm(enumerate(frames)):
            ...
            maskpath = frame_path.replace("images", "masks")
            mask = cv2.imread(maskpath)
            mask = cv2.resize(mask, (self.frame_w, self.frame_h)).copy()
            ...
            c2w = c2w_list[idx]
            rays_o, rays_d = get_rays(direction, c2w, keepdim= True) 
        ...
        self.masks_img : Float[Tensor, "B H W"] = torch.stack(masks_img, dim=0)
        ...
        self.rays_o = torch.stack(self.rays_o, 0)
...
class MultiviewDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        if not (self.cfg.eval_interpolation is None):
            ...
            for idx, frame in tqdm(enumerate(frames)):
                ...
                frames_position.append(camera_position)
                frames_direction.append(direction) 
            self.masks_img : Float[Tensor, "B H W 3"] = torch.stack(masks_img, dim=0)
        threestudio.info("Loaded frames.")
        # self.frames_proj: Float[Tensor, "B 4 4"] = torch.stack(frames_proj, dim=0)
        self.frames_c2w: Float[Tensor, "B 4 4"] = torch.stack(frames_c2w, dim=0)
        self.frames_position: Float[Tensor, "B 3"] = torch.stack(frames_position, dim=0)
    ...
    def __getitem__(self, index):
        if self.cfg.eval_interpolation is None:
            return {
                "index": index,
