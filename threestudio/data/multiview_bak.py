...
from threestudio.data.camera_util import \
...
'''
The resulting matrix is a 4x4 projection matrix that represents the transformation from 
world coordinates to normalized device coordinates. It is commonly used in computer graphics 
and computer vision applications to project 3D points onto a 2D image plane.

Note that the function assumes the input K is a 4x4 matrix, where the top-left 3x3 submatrix 
represents the intrinsic parameters of the camera. The function constructs the projection matrix 
using these intrinsic parameters along with the image dimensions and near/far clipping planes.

It's worth mentioning that this implementation assumes a specific form of the projection matrix 
known as the perspective projection matrix. Other variations, such as orthographic projection, 
may have different forms for the projection matrix.
...
class MultiviewIterableDataset(IterableDataset):
    def __init__(self, cfg: Any) -> None:
        ...
        # frames has the camera position information
        ...
        for frame in tqdm(frames):
            ...
            E_list.append(extrinsic)
            c2w = torch.linalg.inv(extrinsic)
            c2w_list.append(c2w)
        c2w_list = torch.stack(c2w_list, dim=0)
        E_list =torch.stack(E_list, dim=0)

        # If self.cfg.camera_layout is equal to "around", 
        # the code subtracts the mean of the translation vectors (c2w_list[:, :3, 3]) 
        # from each translation vector. This operation centers the camera positions around the origin.
        ...
        for idx, frame in tqdm(enumerate(frames)):
            ...
            #compute the ray directions for each pixel in the image. 
            # The function calculates the direction vector for each pixel based on the intrinsic
            # parameters (intrinsic) and the image dimensions
            ...
            R = E[:3, :3]
            T = E[:3, 3]
            rays_o, rays_d = get_rays_from_KRT(self.frame_h, self.frame_w, intrinsic[:3,:3].numpy(), R, T)
            frames_rays_d.append(torch.from_numpy(rays_d))
            frames_rays_o.append(torch.from_numpy(rays_o))
        ...
        self.rays_o : Float[Tensor, "B H W 3"] = torch.stack(frames_rays_o, dim=0)
        self.rays_d : Float[Tensor, "B H W 3"] = torch.stack(frames_rays_d, dim=0)
        # self.rays_o, self.rays_d = get_rays(self.frames_direction, self.frames_c2w, keepdim=True) # [30, 512, 512,3]
